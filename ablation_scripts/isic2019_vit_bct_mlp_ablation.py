# -*- coding: utf-8 -*-
"""
Medical Image Ablation Study: ViT + BCT + MLP on ISIC 2019

Author: Prof. Dr. Metin Zontul
Date: September 2026
Description: 
- Evaluates the clinical resilience of the standard MLP classifier head under 
...
# =============================================================================
# 2. INTEGRATED CUSTOM MODULES (BCT)
# =============================================================================
...
# =============================================================================
# 3. MODEL ARCHITECTURES (MLP HEADS)
# =============================================================================

class ViT_Baseline_MLP(nn.Module):
...
class ViT_Proposed_BCT_MLP(nn.Module):
    def __init__(self, num_classes, compressed_dim=128):
        super().__init__()
        self.backbone = vit_b_16(weights=ViT_B_16_Weights.IMAGENET1K_V1)
        self.backbone.heads = nn.Identity()
        self.bct = BCT(in_features=768, out_features=compressed_dim) 
        self.mlp = nn.Linear(in_features=compressed_dim, out_features=num_classes)
        
    def forward(self, x):
        return self.mlp(self.bct(self.backbone(x)))
...
def count_head_parameters(model, is_proposed=False):
    if is_proposed:
        return sum(p.numel() for p in model.bct.parameters() if p.requires_grad) + \
               sum(p.numel() for p in model.mlp.parameters() if p.requires_grad)
    else:
...
if __name__ == "__main__":
    print("=== [ISIC 2019] ViT+BCT+MLP ABLATION STUDY STARTING ===")
    ...
    for i, seed in enumerate(seeds):
        print(f"\n[ISIC 2019] Baseline ViT + MLP (Run {i+1}/{len(seeds)})")
        set_seed(seed)
        model = ViT_Baseline_MLP(num_classes).to(device)
    ...
    for i, enumerate_seed in enumerate(seeds):
        print(f"\n[ISIC 2019] Proposed ViT + BCT + MLP (Run {i+1}/{len(seeds)})")
        set_seed(enumerate_seed)
        model = ViT_Proposed_BCT_MLP(num_classes, compressed_dim=128).to(device)
        acc, f1, prec, rec = train_model(model, trainloader, valloader, class_weights, device, epochs)
    ...
    print("-----------------------------------------------------------------------------------------------------------------------------------------")
    print(f"ViT+MLP (Baseline)     | {np.mean(base_accs):.2f}±{np.std(base_accs):.2f} | {np.mean(base_f1s):.4f}±{np.std(base_f1s):.4f} | {np.mean(base_precs):.4f}±{np.std(base_precs):.4f} | {np.mean(base_recs):.4f}±{np.std(base_recs):.4f} | {np.mean(latencies['Baseline']):.2f} ms     | {base_params:,}     | -")
    print(f"ViT+BCT+MLP (Proposed) | {np.mean(prop_accs):.2f}±{np.std(prop_accs):.2f} | {np.mean(prop_f1s):.4f}±{np.std(prop_f1s):.4f} | {np.mean(prop_precs):.4f}±{np.std(prop_precs):.4f} | {np.mean(prop_recs):.4f}±{np.std(prop_recs):.4f} | {np.mean(latencies['Proposed']):.2f} ms     | {prop_params:,}      | {compression_rate:.1f}%")
    print("-----------------------------------------------------------------------------------------------------------------------------------------")
